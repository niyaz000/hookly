...
pub async fn update_schedule(
    ...
    let schedule = service(state).update(public_id, payload, make_ctx()).await?;
    Ok(Json(schedule))
}
...
pub async fn restore_schedule(
    ...
    let schedule = service(state).restore(public_id, make_ctx()).await?;
    Ok(Json(schedule))
}
...
pub async fn pause_schedule(
    ...
    let schedule = service(state).pause(public_id, make_ctx()).await?;
    Ok(Json(schedule))
}
...
pub async fn resume_schedule(
    ...
    let schedule = service(state).resume(public_id, make_ctx()).await?;
    Ok(Json(schedule))
}
...
pub async fn trigger_schedule(
    ...
    let execution = service(state).trigger(public_id, make_ctx()).await?;
